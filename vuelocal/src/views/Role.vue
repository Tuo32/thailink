...
export default {
  ...
  methods: {
    ...
    selectMenu(roleID){
      ...
      this.roleID = roleID;
      // this.request.get("http://localhost/menu").then(res=>{
      //   this.menuData = res.data
      //   this.expands = this.menuData.map(v => v.roleID)
      // })

      //get the current menu authority and set as default checked in the check list
      this.request.get("http://"+this.$server+":9090/role/roleMenu/" + roleID).then(res => {
    ...
    load() {
      request.get("http://"+this.$server+":9090/role/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&roleID=" + this.roleIDSearch + "&roleName=" + this.roleName).then(res => {
        console.log(res)
    ...
    save() {
      request.post("http://"+this.$server+":9090/role", this.form).then(res => {
    ...
    saveRoleMenu(){
     ...
     this.request.post("http://"+this.$server+":9090/role/roleMenu/" + this.roleID,this.$refs.tree.getCheckedKeys()).then(res => {
       if (res.code == 'CODE_200') {
         this.$message.success("saved successfully")
         this.dialogFormVisible = false
         this.load()
    ...
    del(id) {
      request.delete("http://"+this.$server+":9090/role/" + id).then(res => {
        if (res.code == 'CODE_200') {
    ...
    delBatch() {
      ...
      request.post("http://"+this.$server+":9090/role/del/batch", ids).then(res => {
        if (res.code == 'CODE_200') {
          this.$message.success("deleted successfully")
          this.dialogFormVisible = false
          this.load()
