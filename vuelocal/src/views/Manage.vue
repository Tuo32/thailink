...
export default {
  ...
  methods: {
    ...
    save(){
      request.post("http://"+this.$server+":/user", this.form).then(res =>{
        if(res){
    ...
    del(id){
      request.delete("http://"+this.$server+":/user/" + id).then(res => {if(res){this.$message.success("删除成功")
      this.load()}
      else{
    ...
    delBatch(){
      ...
      request.post("http://"+this.$server+":/user/del/batch", ids).then(res => {if(res){this.$message.success("批量删除成功")
        this.load()}
      else{
