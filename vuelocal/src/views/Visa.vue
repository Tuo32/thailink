...
//variables for the customer table in 'add new dialog'
  ...
  methods: {
    ...
    updateStatus(visaID, status){
      request.post("http://"+this.$server+":9090/visa/statusUpdate?visaID=" + visaID + "&status=" + status + "&requesterID=" + this.currentUser.id).then(res => {
    ...
    revertStatus(visaID, status){
      request.post("http://"+this.$server+":9090/visa/statusRevert?visaID=" + visaID + "&status=" + status + "&requesterID=" + this.currentUser.id).then(res => {
        if (res.code == 'CODE_200') {
    ...
    fetch(visaID){
      ...
      request.get("http://"+this.$server+":9090/visa/visa-service?visaID="+visaID).then(res =>{
            console.log(res)
          this.join = res
      }
      )
     ...
     load() {
      ...
      this.$notify.closeAll()
      request.get("http://"+this.$server+":9090/visa/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&visaID=" + this.visaID + "&visaType="+ this.searchVisaType + "&userID="+ this.searchVisaType + "&updatedOrder=" + this.updatedOrder).then(async res => {
    ...
    save() {
      console.log("http://"+this.$server+":9090/visa/save?requesterID="+this.currentUser.id, this.form)

      request.post("http://"+this.$server+":9090/visa/save?requesterID="+this.currentUser.id, this.form).then(res => {
        console.log(res)
        if (res.code == 'CODE_200') {
    ...
    del(id) {
      request.delete("http://"+this.$server+":9090/visa/del?id=" + id + "&requesterID=" +this.currentUser.id).then(res => {
        if (res) {
    ...
    delBatch() {
      ...
      request.post("http://"+this.$server+":9090/visa/del/batch?requesterID="+this.currentUser.id, ids).then(res => {
        if (res) {
