...
export default {
  ...
  //this is used to remove the customer's private information
    privacy(){
      ...
      request.post("http://"+this.$server+":9090/customer/clear?token="+this.user.token, ids).then(res => {
    ...
    load() {
      request.get("http://"+this.$server+":9090/customer/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&customerID=" + this.customerID + "&name=" + this.name + "&token=" + this.user.token).then(res => {
    ...
    save() {
      ...
      request.post("http://"+this.$server+":9090/customer?token=" + this.user.token, this.form).then(res => {
        if (res.code == 'CODE_200') {
    ...
    del(id) {
      request.delete("http://"+this.$server+":9090/customer/delete?id=" + id + "&token=" + this.user.token).then(res => {
    ...
    delBatch() {
      let ids = this.multipleSelection.map(v => v.customerID)
      request.post("http://"+this.$server+":9090/customer/del/batch?token=" + this.user.token, ids).then(res => {
        if (res.code == 'CODE_200') {
          this.$message.success("deleted successfully")
          this.load()
        }else{
