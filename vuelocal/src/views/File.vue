<template>
  <div>
    ...
    <div style="">
      ...
      <el-upload :action="'http://' + this.server + ':9090/file/upload?serviceID='+this.uploadServiceID" :show-file-list="false"
...
export default {
  ...
  methods: {
    ...
    load() {
      request.get("http://"+this.$server+":9090/file/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&name=" + this.name + "&serviceID=" + this.serviceID).then(res => {
        console.log(res)
    ...
    del(fileID) {
      request.delete("http://"+this.$server+":9090/file/" + fileID).then(res => {
        if (res) {
    ...
    delBatch() {
      ...
      request.post("http://"+this.$server+":9090/file/del/batch", ids).then(res => {
        if (res) {
    ...
    donwload(url){
      axios({
        method: 'get',
        url: 'http://'+this.$server+':9090/file/download',
        responseType: 'blob',
        headers:this.headerObj,
        params: {
          fileUUID: url
