...
export default {
  ...
  methods: {
    ...
    load() {
      request.get("http://"+this.$server+":9090/service/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&serviceID=" + this.serviceID + "&customerID=" + this.customerIDSearch + "&serviceType=" + this.serviceType + "&infoID=" + this.infoID).then(res => {
        console.log(res)
    ...
    save() {
      request.post("http://"+this.$server+":9090/service/save?requesterID="+this.currentUser.id, this.form).then(res => {
    ...
    // update() {
    //   request.post("http://localhost/service/update", this.form).then(res => {
    //     console.log(res.code)
    //     if (res.code == 'CODE_200') {
    //       this.$message.success("saved successfully")
    //       this.dialogFormVisible = false
    ...
    // },
    del(id) {
      request.delete("http://"+this.$server+":9090/service/del?id=" + id + "&requesterID=" +this.currentUser.id).then(res => {
       console.log(res)
        if (res.code == 'CODE_200') {
    ...
    delBatch() {
      ...
      request.post("http://"+this.$server+":9090/service/del/batch?requesterID="+this.currentUser.id, ids).then(res => {
        if (res.code == 'CODE_200') {
          this.$message.success("deleted successfully")
          this.load()
        }else{
    ...
    // history() {
    //   let ids = this.multipleSelection.map(v => v.serviceID)
    //   request.post("http://"+this.$server+":9090/service/history?requesterID="+this.currentUser.id, ids).then(res => {
    //     if (res.code == 'CODE_200') {
