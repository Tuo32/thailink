...
export default {
  ...
  methods: {
    ...
    load() {
      request.get("http://"+this.$server+":9090/finance/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&serviceID=" +this.serviceID + "&createTime=" +this.createTime ).then(res => {
        console.log(res)
    ...
    save() {
      request.post("http://"+this.$server+":9090/finance/save?requesterID=" + this.currentUser.id, this.form).then(res => {
    ...
    update() {
      request.post("http://"+this.$server+":9090/finance/update", this.form).then(res => {
        console.log(res.code)
        if (res.code == 'CODE_200') {
          this.$message.success("saved successfully")
          this.dialogFormVisible = false
    ...
    del(id) {
      request.delete("http://"+this.$server+":9090/finance/del?id=" + id + "&requesterID=" + this.currentUser.id).then(res => {
    ...
    delBatch() {
      ...
      request.post("http://"+this.$server+":9090/finance/del/batch?requesterID=" + this.currentUser.id, ids).then(res => {
        if (res) {
    ...
    generateReceipt() {
      let ids = this.multipleSelection.map(v => v.financeID)
      request.post("http://"+this.$server+":9090/pdf/generate-receipt?requesterID=" + this.currentUser.id, ids)
          .then(res => {
    ...
    download(fileName){
      request.get("http://"+this.$server+":9090/pdf/" + fileName).then(res => {
        if (res) {
          // window.open(res)
