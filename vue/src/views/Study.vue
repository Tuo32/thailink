<template>
  <div>
    <div style="padding:10px 0">
      ...
      <el-input style="width: 200px" suffix-icon="el-icon-search"
                v-model="userID" placeholder="User ID" v-if="studyViewAllVisible">
      </el-input>
...
export default {
  ...
  created() {
    ...
    if(this.storeMenus.indexOf("Study View All") != -1){
      this.studyViewAllVisible = true;
      this.userID = "";
    }
    this.load()
  },
  methods: {
    ...
    load() {
      ...
      request.get("http://"+this.$server+"/api/study/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&studyID=" + this.studyID + "&userID=" + this.userID).then(async res => {
    ...
    fetch(studyID){
      ...
      request.get("http://"+this.$server+"/api/study/study-service?studyID="+studyID).then(res =>{
            console.log(res)
            this.join = res
          }
      )
    ...
    updateStatus(studyID, status){
      request.post("http://"+this.$server+"/api/study/statusUpdate?studyID=" + studyID + "&status=" + status + "&requesterID=" + this.currentUser.id ).then(res => {
    ...
    revertStatus(studyID, status){
      request.post("http://"+this.$server+"/api/study/statusRevert?studyID=" + studyID + "&status=" + status + "&requesterID=" + this.currentUser.id).then(res => {
        if (res.code == 'CODE_200') {
          this.$message.success("status proceeded")
          this.dialogFormVisible = false
          this.load()
    ...
    save() {
      request.post("http://"+this.$server+"/api/study/save?requesterID="+this.currentUser.id, this.form).then(res => {
        console.log(res)
        if (res.code == 'CODE_200') {
    ...
    del(id) {
      request.delete("http://"+this.$server+"/api/study/del?id=" + id + "&requesterID=" +this.currentUser.id).then(res => {
        if (res) {
    ...
    delBatch() {
      ...
      request.post("http://"+this.$server+"/api/study/del/batch?requesterID="+this.currentUser.id, ids).then(res => {
        if (res) {
          this.$message.success("succeed")
          this.load()
        } else {
    ...
    loadCustomer() {
      request.get("http://"+this.$server+"/api/customer/page?pageNum=" + this.pageNumCustomer + "&pageSize=" + this.pageSizeCustomer + "&customerID=" + this.customerID + "&name=" + this.customerName + "&status=" + this.status ).then(res => {
