<template>
  <div>
    <div style="padding:10px 0">
      ...
      <el-input style="width: 150px" suffix-icon="el-icon-search"
                v-model="userID" placeholder="User ID" v-if="visaViewAllVisible">
      </el-input>
      <el-select v-model="searchVisaType" style="width: 150px">
        ...
        <!--            <el-option label="Investment visa" value="Investment visa"></el-option>-->
...
<!--          @confirm = "delBatch"-->
<!--      >-->
<!--        <el-button type="danger" slot="reference" v-if="visaDeleteVisible" >Batch delete </el-button>-->
<!--      </el-popconfirm>-->
...
<!--              icon-color="red"-->
<!--              title="您确定删除吗？"-->
<!--              @confirm = "del(scope.row.visaID)"-->
<!--            <el-button type="danger" slot="reference" style="margin-top: 5px; margin-right: 5px" v-if="visaDeleteVisible">Delete  </el-button>-->
<!--          </el-popconfirm>-->

          <el-button  type="info" @click="fetch(scope.row.visaID)" style="margin-top: 5px; ">Details</el-button>

<!--          <el-button  type="warning" @click="updateStatus(scope.row.visaID, scope.row.status)" style="margin-top: 5px; margin-left: 0px;" v-if="statusUpdateVisible">Proceed</el-button>-->
                <el-popconfirm
                    class="ml-5"
...
<!--          <el-button  type="warning" @click="revertStatus(scope.row.visaID, scope.row.status)" style="margin-top: 5px; margin-left: 5px;" v-if="statusUpdateVisible">Revert</el-button>-->
    ...
    <el-dialog title="Visa Record Fetch Information" :visible.sync="dialogFormVisible2" width="30%">
      ...
      <div slot="footer" class="dialog-footer">
        <el-upload :action="'http://' + this.server + '/file/upload?serviceID='+this.join.serviceID" :show-file-list="false"
                   :on-success="handleFileUploadSuccess" :on-error="uploadError" style="display: inline-block">
...
//variables for the customer table in 'add new dialog'
  ...
  created() {
    ...
    if(this.storeMenus.indexOf("Visa Delete") != -1){
      this.visaDeleteVisible = true;
    }
    if(this.storeMenus.indexOf("Visa Edit") != -1){
      this.visaEditVisible = true;
    }
    if(this.storeMenus.indexOf("Visa View All") != -1){
      this.visaViewAllVisible = true;
      this.userID = "";
    }
    this.load()
  ...
  methods: {
    ...
    updateStatus(visaID, status){
      request.post("http://"+this.$server+"/api/visa/statusUpdate?visaID=" + visaID + "&status=" + status + "&requesterID=" + this.currentUser.id).then(res => {
    ...
    revertStatus(visaID, status){
      request.post("http://"+this.$server+"/api/visa/statusRevert?visaID=" + visaID + "&status=" + status + "&requesterID=" + this.currentUser.id).then(res => {
        if (res.code == 'CODE_200') {
          this.$message.success("status proceeded")
          this.dialogFormVisible = false
          this.load()
    ...
    fetch(visaID){
      ...
      request.get("http://"+this.$server+"/api/visa/visa-service?visaID="+visaID).then(res =>{
            console.log(res)
          this.join = res
      }
      )
     ...
     load() {
      ...
      this.$notify.closeAll()
      request.get("http://"+this.$server+"/api/visa/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&visaID=" + this.visaID + "&visaType="+ this.searchVisaType + "&userID=" + this.userID).then(async res => {
    ...
    save() {
      console.log("http://"+this.$server+"/api/visa/save?requesterID="+this.currentUser.id, this.form)

      request.post("http://"+this.$server+"/api/visa/save?requesterID="+this.currentUser.id, this.form).then(res => {
        console.log(res)
        if (res.code == 'CODE_200') {
    ...
    del(id) {
      request.delete("http://"+this.$server+"/api/visa/del?id=" + id + "&requesterID=" +this.currentUser.id+ "&token=" + this.user.token).then(res => {
        if (res) {
    ...
    delBatch() {
      let ids = this.multipleSelection.map(v => v.visaID)
      request.post("http://"+this.$server+"/api/visa/del/batch?requesterID="+this.currentUser.id + "&token=" + this.user.token, ids).then(res => {
        if (res) {
    ...
    loadCustomer() {
      request.get("http://"+this.$server+"/api/customer/page?pageNum=" + this.pageNumCustomer + "&pageSize=" + this.pageSizeCustomer + "&customerID=" + this.customerID + "&name=" + this.customerName + "&status=" + this.status + "&token=" + this.user.token ).then(res => {
