<template>
  <div>
    ...
    <div style="">
      ...
      <el-upload :action="'http://' + this.server + '/file/upload?serviceID='+this.uploadServiceID" :show-file-list="false"
...
<!--          @confirm = "delBatch"-->
<!--      >-->
<!--        <el-button type="danger" slot="reference"  v-if="fileDeleteVisible">Batch delete </el-button>-->
<!--      </el-popconfirm>-->
...
<!--      </el-table-column>-->
      ...
      <el-table-column>
        <template v-slot="scope">
          ...
          >
            <el-button class="ml-5" type="danger" slot="reference" style="margin-top: 5px" v-if="fileDeleteVisible" >删除  </el-button>
          </el-popconfirm>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    load() {
      request.get("http://"+this.$server+"/api/file/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&name=" + this.name + "&serviceID=" + this.serviceID).then(res => {
        console.log(res)
    ...
    del(fileID) {
      request.delete("http://"+this.$server+"/api/file/" + fileID).then(res => {
        if (res) {
    ...
    delBatch() {
      ...
      request.post("http://"+this.$server+"/api/file/del/batch", ids).then(res => {
        if (res) {
    ...
    donwload(url){
      axios({
        method: 'get',
        url: 'http://'+this.$server+'/file/download',
        responseType: 'blob',
        headers:this.headerObj,
        params: {
          fileUUID: url
      ...
      // request.get("http://"+this.$server+"/api/file/download?fileUUID=" + url).then(res => {
      //   if (res) {
      //     // window.open(url)
