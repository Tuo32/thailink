...
export default {
  ...
  created() {
    // after the first login, the router is not loaded. Refresh to load the router
    if (!localStorage.getItem('reloaded')) {
      localStorage.setItem('reloaded', 'true');
      // this.$router.replace( 'http://thailink.japaneast.cloudapp.azure.com/#/home' });
  ...
  methods:{
    pieChartConfigure(){
      request.get("http://"+this.$server+"/api/chart/pie").then(res => {
    ...
    financeLineChartConfigure(){
      request.get("http://"+this.$server+"/api/finance/chart/line?startDate=" + this.financeLineChartStartDate + "&endDate=" + this.financeLineChartEndDate ).then(res => {
  ...
  return currentdate;
    ...
    financeSummary(){
      request.get("http://"+this.$server+"/api/finance/sum?startDate=" + this.financeLineChartStartDate + "&endDate=" + this.financeLineChartEndDate ).then(res => {
