...
export default {
  ...
  methods: {
    ...
    selectMenu(roleID){
      ...
      this.request.get("https://"+this.$server+"/role/roleMenu/" + roleID).then(res => {
    ...
    load() {
      request.get("https://"+this.$server+"/role/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&roleID=" + this.roleIDSearch + "&roleName=" + this.roleName).then(res => {
        console.log(res)
    ...
    save() {
      request.post("https://"+this.$server+"/role", this.form).then(res => {
    ...
    saveRoleMenu(){
     ...
     this.request.post("https://"+this.$server+"/role/roleMenu/" + this.roleID,this.$refs.tree.getCheckedKeys()).then(res => {
       if (res.code == 'CODE_200') {
         this.$message.success("saved successfully")
         this.dialogFormVisible = false
         this.load()
    ...
    del(id) {
      request.delete("https://"+this.$server+"/role/" + id).then(res => {
        if (res.code == 'CODE_200') {
    ...
    delBatch() {
      ...
      request.post("https://"+this.$server+"/role/del/batch", ids).then(res => {
        if (res.code == 'CODE_200') {
          this.$message.success("deleted successfully")
          this.dialogFormVisible = false
          this.load()
