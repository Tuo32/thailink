<template>
<!--  this page is the "outfit" to contain sub pages-->
...
export default {
  ...
  methods: {
    ...
    save(){
      request.post("https://"+this.$server+":/user", this.form).then(res =>{
        if(res){
    ...
    del(id){
      request.delete("https://"+this.$server+":/user/" + id).then(res => {if(res){this.$message.success("删除成功")
      this.load()}
      else{
    ...
    delBatch(){
      ...
      request.post("https://"+this.$server+":/user/del/batch", ids).then(res => {if(res){this.$message.success("批量删除成功")
        this.load()}
      else{
