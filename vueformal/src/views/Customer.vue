...
<!--customer data table-->
    ...
    <el-table :data="tableData"   stripe  border style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="birthPlace" label="Birth Place">
      </el-table-column>
      ...
      <el-table-column prop="modifyTime" label="Modified" width="120">
      </el-table-column>
    ...
    <el-dialog title="Customer Information" :visible.sync="dialogFormVisible" width="30%">
...
export default {
  ...
  data(){
    return {
      ...
      pageNum: 1,
      pageSize: 5,
      //obj to contain customer data, used for add new customer and edit customer
      form:{},
      //check box
      multipleSelection:[],
      //add new form visibility
      dialogFormVisible:false,
      //this is used to check the url and permission available
      storeMenus:localStorage.getItem("menus"),
      //used for button availability
  ...
  //this is used to remove the customer's private information
    privacy(){
      let ids = this.multipleSelection.map(v => v.customerID)
      request.post("https://"+this.$server+"/customer/clear?token="+this.user.token, ids).then(res => {
    ...
    load() {
      request.get("https://"+this.$server+"/customer/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&customerID=" + this.customerID + "&name=" + this.name + "&token=" + this.user.token).then(res => {
    ...
    //validate the data and save the new customer record
    save() {
      ...
      request.post("https://"+this.$server+"/customer?token=" + this.user.token, this.form).then(res => {
        if (res.code == 'CODE_200') {
    ...
    del(id) {
      request.delete("https://"+this.$server+"/customer/delete?id=" + id + "&token=" + this.user.token).then(res => {
    ...
    delBatch() {
      let ids = this.multipleSelection.map(v => v.customerID)
      request.post("https://"+this.$server+"/customer/del/batch?token=" + this.user.token, ids).then(res => {
        if (res.code == 'CODE_200') {
          this.$message.success("deleted successfully")
          this.load()
        }else{
