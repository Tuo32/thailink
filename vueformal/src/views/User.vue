...
export default {
  ...
  methods: {
    ...
    load() {
      request.get("https://"+this.$server+"/user/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&userID=" + this.userID + "&username=" + this.username+"&name="+this.name+"&roleID="+this.roleID ).then(res => {
    ...
    save() {
      ...
      request.post("https://"+this.$server+"/user/save?requesterID="+this.currentUser.id , this.form).then(res => {
        console.log(res)
    ...
    del(id) {
      request.delete("https://"+this.$server+"/user/del?id=" + id + "&requesterID=" +this.currentUser.id + "&token=" + this.currentUser.token).then(res => {
        if (res.code == 'CODE_200') {
    ...
    delBatch() {
      ...
      request.post("https://"+this.$server+"/user/del/batch?requesterID="+this.currentUser.id + "&token=" + this.currentUser.token, ids).then(res => {
        if (res.code == 'CODE_200') {
          this.$message.success("deleted successfully")
          this.load()
        }else{
    ...
    loadRole() {
      request.get("https://"+this.$server+"/role/page?pageNum=" + 1 + "&pageSize=" + 10 + "&roleID=" + this.roleIDAdd + "&roleName=" + this.roleName).then(res => {
        this.tableDataRole = res.records
      })
    },
    //load the data from role table to role filter
    loadRoleForFilter(){
      request.get("https://"+this.$server+"/role/all").then(res=>{
