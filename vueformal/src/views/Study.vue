<template>
  <div>
    ...
    <el-table :data="tableData"  :stripe="true"  border  style="width: 100%; " @selection-change="handleSelectionChange" >
      ...
      <el-table-column prop="formerSchool" label="Former School" width="180">
      </el-table-column>
      <el-table-column prop="applySchool" label="Apply School" width="180">
      </el-table-column>
    ...
    <el-dialog title="Study Record Fetch Information" :visible.sync="dialogFormVisible2" width="30%">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        >
          <el-button type="danger" slot="reference" v-if="studyEditVisible">Archive </el-button>
        </el-popconfirm>
        <el-upload :action="'https://' + this.server + '/file/upload?serviceID='+this.join.serviceID" :show-file-list="false"
          ...
          <el-button type="primary" class="ml-5" v-if="studyEditVisible">Upload File</el-button>
        </el-upload>
...
export default {
  ...
  methods: {
    ...
    load() {
      ...
      request.get("https://"+this.$server+"/study/page?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize + "&studyID=" + this.studyID + "&userID="+ this.userID + "&applySchool="+ this.applySchool + "&updatedOrder=" + this.updatedOrder).then(async res => {
    ...
    fetch(studyID){
      ...
      request.get("https://"+this.$server+"/study/study-service?studyID="+studyID).then(res =>{
            console.log(res)
            this.join = res
          }
      )
    ...
    updateStatus(studyID, status){
      request.post("https://"+this.$server+"/study/statusUpdate?studyID=" + studyID + "&status=" + status + "&requesterID=" + this.currentUser.id ).then(res => {
    ...
    revertStatus(studyID, status){
      request.post("https://"+this.$server+"/study/statusRevert?studyID=" + studyID + "&status=" + status + "&requesterID=" + this.currentUser.id).then(res => {
        if (res.code == 'CODE_200') {
    ...
    save() {
      request.post("https://"+this.$server+"/study/save?requesterID="+this.currentUser.id, this.form).then(res => {
        console.log(res)
        if (res.code == 'CODE_200') {
